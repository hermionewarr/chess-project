...
//local funtions
template<class T> void print_vector(std::vector<T> const& vec) {
	for (T element : vec) {
...
std::shared_ptr<piece> board::operator[](int index) const
{
	try {
		return current_board[index];
	}
	catch (const std::out_of_range& e) {
		std::cerr << "Error: trying to access vector element out of bounds" << e.what() << std::endl;
		return NULL;
	}
}
//functions to turn the entered locations into row and column values that 
//can be put into the index function and vice versa
int board::string_to_file(std::string location) const {
	int file = (int)tolower(location[0]) - 96;//column
	return file;
}
int board::string_to_rank(std::string location) const {
	int rank = 9 - (location[1] - '0'); //row
	return rank;
}
int board::string_to_index(std::string location) const {
	int board_index = index(string_to_rank(location), string_to_file(location));
	return board_index;
}
std::string board::string_coordinate(const int vector_index) const {
...
//entrance point to the game
void board::user_input() {
	std::string option;
	std::cout << "Options:  New game<N>  Move<M>  Quit<Q>  Undo move<U>  Load game<L>  Save game<S> \nAnswer: ";
	std::cin >> option;
...
void board::move_input() {
	...
	if (piece_location.length() == 2 && string_to_index(piece_location) != 100) {//check valid entry
		//checking right players turn, black or white. Can't select empty, colour 2.
		if (current_turn == current_board[string_to_index(piece_location)]->piece_colour()) {
			std::cout << "Piece selected: ";
			std::cout << current_board[string_to_index(piece_location)]->name() << std::endl;
		}
		else if (current_board[string_to_index(piece_location)]->piece_colour() == 2) {
			std::cout << "You have not selected a piece. Try again." << std::endl;
			user_input();
			return;
		}
		else if (current_turn != current_board[string_to_index(piece_location)]->piece_colour()) {
	...
	if (piece_destination.length() == 2 && string_to_index(piece_destination) != 100) {
		...
		if (is_path_clear(piece_location, piece_destination) == true) {
...
bool board::move(std::string location, std::string destination) {
	...
	int file_move = string_to_file(piece_destination) - string_to_file(piece_location);
	//else check if its a king and rook for castling op
	if (current_board[string_to_index(piece_location)]->name() == "King" && current_board[string_to_index(piece_destination)]->icon() == '-' && 
		king_in_check() == false && abs(string_to_file(piece_destination) - string_to_file(piece_location)) == 2) {
	...
	//check if destination square is empty
	else if (current_board[string_to_index(piece_destination)]->icon() == '-') {
		if (current_board[string_to_index(piece_location)]->valid_move(piece_location, piece_destination, 0)) {
			if (current_board[string_to_index(piece_location)]->name() == "Pawn" && (string_to_rank(piece_destination) == 8 || string_to_rank(piece_destination) == 1)) {
				//pawn promotion
		...
		//en passant
		else if(current_board[string_to_index(piece_location)]->name() == "Pawn" && current_board[(string_to_index(piece_location) + file_move)]->name() == "Pawn" &&
				current_board[string_to_index(piece_location)]->valid_move(piece_location, piece_destination, 1) &&
				current_board[(string_to_index(piece_location) + file_move)]->number_of_moves_made() == 1) {
			en_passant(piece_location, piece_destination);
		}
		else {
	...
	else if (current_board[string_to_index(piece_destination)]->piece_colour() != current_board[string_to_index(piece_location)]->piece_colour()) {
		if (current_board[string_to_index(piece_location)]->valid_move(piece_location, piece_destination, 1)) {
			if (current_board[string_to_index(piece_location)]->name() == "Pawn" && (string_to_rank(piece_destination) == 8 || string_to_rank(piece_destination) == 1)) {
				//pawn promotion
	...
	if (king_in_check() == true && checkmate() == true) {
		game_over = true;
		std::cout << "\nDo you want to save this game? (y/n)" << std::endl;
		std::string save;
		std::cin >> save;
		if (save == "y" || save == "Y") {
			save_game();
...
bool board::castling_possible(std::string king_location, std::string king_destination) {
	...
	std::string rook_position = string_coordinate((string_to_index(king_location) + rook_file_position));
	//neither piece can have moved before. Can't be any pieces inbetween. Can't castle out of check
	if (current_board[string_to_index(king_location)]->number_of_moves_made() == 0 &&
		current_board[(string_to_index(king_location)+ rook_file_position)]->number_of_moves_made() == 0 && is_path_clear(king_location, rook_position)) {
		return true;
	}
	return false;
}
			...
			for (int i{ string_to_rank(piece_location) }; i < string_to_rank(piece_destination); i++) {
				...
				if (string_to_file(piece_destination) > string_to_file(piece_location)) {
					for (int j{ string_to_file(piece_location) }; j < string_to_file(piece_destination); j++) {
						if (current_board[index(i, j)]->icon() != '-' && current_board[string_to_index(piece_location)]->valid_move(piece_location, string_coordinate(index(i, j)), 0) == true
							&& abs(string_to_rank(string_coordinate(index(i, j))) - string_to_rank(piece_location)) == abs(string_to_file(string_coordinate(index(i, j))) - string_to_file(piece_location))) {
							//std::cout << "testy boi";
							path_is_clear = false;
						}
...
bool board::king_in_check() {
	...
	for (std::vector<std::shared_ptr<piece>>::iterator it = current_board.begin(); it != current_board.end(); it++, vector_index++) {
		...
		if ((*it)->name() == "King" && (*it)->piece_colour() == current_turn) {
			king_location = string_coordinate(vector_index);
		}
	}
	int index{};
	...
	for (std::vector<std::shared_ptr<piece>>::iterator it = current_board.begin(); it != current_board.end(); it++, index++) {
		if ((*it)->piece_colour() != current_turn && (*it)->valid_move(string_coordinate(index), king_location, 1)
			&& is_path_clear(string_coordinate(index), king_location)) {
			return 1;
		}
	}
	return 0;
...
void board::undo_move() {
	...
	if (move_type == '-') {
		...
		//undo take
		swap_piece(piece_destination, piece_location, undo);
		current_board[string_to_index(piece_destination)] = taken_pieces.back();
		...
		if (string_to_file(piece_destination) > 5) {//king side
			current_board[(string_to_index(piece_location) + 3)] = current_board[(string_to_index(piece_location) + 1)];
			current_board[(string_to_index(piece_location) + 1)] = std::make_shared<empty>();
		}
		else {//queen side
			current_board[(string_to_index(piece_location) - 4)] = current_board[(string_to_index(piece_location) - 1)];
			current_board[(string_to_index(piece_location) - 1)] = std::make_shared<empty>();
		}
...
bool board::checkmate() {
	...
	for (std::vector<std::shared_ptr<piece>>::iterator it = current_board.begin(); it != current_board.end(); it++, i++) {
		if ((*it)->piece_colour() == current_turn) {
			std::string it_location = string_coordinate(i);
...
std::vector<std::string> board::help(std::string location) {
	...
	int location_index = string_to_index(location);
	for (std::vector<std::shared_ptr<piece>>::iterator it_des = current_board.begin(); it_des != current_board.end(); it_des++, j++) {
		std::string destination = string_coordinate(j);
		int file_move = string_to_file(destination) - string_to_file(location);
		if (location != destination) {
			...
			if ((*it_des)->piece_colour() != current_turn && is_path_clear(location, destination) == true) {
				...
				else if ((*it_des)->icon() == '-'&& current_board[location_index]->name() == "Pawn" && current_board[(location_index + file_move)]->name() == "Pawn" && current_board[(location_index + file_move)]->piece_colour() != current_board[location_index]->piece_colour()
					&& current_board[(location_index + file_move)]->number_of_moves_made() == 1 && (string_to_rank(location) == 4 || string_to_rank(location) == 5) && current_board[string_to_index(location)]->valid_move(location,destination,1)) {
					//en passant
					en_passant(location, destination);
					if (king_in_check() == false) { destinations.push_back(destination); }
					undo_move();
				}
...
void board::swap_piece(std::string location, std::string destination, bool undo_move) {
	...
	if (undo_move == false) {
		current_board[string_to_index(location)]->piece_move();
		list_of_moves_made.push_back(std::string(1, current_board[string_to_index(location)]->icon()) + ":" + location + "->-" + destination);
	}
	else if (undo_move == true && list_of_moves_made.size() != 0) {
		current_board[string_to_index(location)]->undo_piece_move();
		list_of_moves_made.pop_back();
	}
	std::shared_ptr<piece> temp = current_board[string_to_index(destination)];
	current_board[string_to_index(destination)] = current_board[string_to_index(location)];
	current_board[string_to_index(location)] = temp;
}
void board::take_piece(std::string location, std::string destination) {
	...
	list_of_moves_made.push_back(std::string(1, current_board[string_to_index(location)]->icon()) + ":" + location + "->X" + destination);
	current_board[string_to_index(location)]->piece_move();
...
void board::castling(std::string location, std::string destination) {
	list_of_moves_made.push_back(std::string(1, current_board[string_to_index(location)]->icon()) + ":" + location + "->C" + destination);
	current_board[string_to_index(location)]->piece_move();
	//move the king
	current_board[string_to_index(destination)] = current_board[string_to_index(location)];
	current_board[string_to_index(location)] = std::make_shared<empty>();
	...
	if (string_to_file(destination) > 5) {//king side
		current_board[(string_to_index(location) + 1)] = current_board[(string_to_index(location) + 3)];
		current_board[(string_to_index(location) + 3)] = std::make_shared<empty>();
	}
	else {//queen side
		current_board[(string_to_index(location) - 1)] = current_board[(string_to_index(location) - 4)];
		current_board[(string_to_index(location) - 4)] = std::make_shared<empty>();
	}
}
...
void board::en_passant(std::string location, std::string destination) {
	//pawn moves diagonally to an empty and takes the pawn behind it
	list_of_moves_made.push_back(std::string(1, current_board[string_to_index(location)]->icon()) + ":" + location + "->E" + destination);
	current_board[string_to_index(location)]->piece_move();
	current_board[string_to_index(destination)] = current_board[string_to_index(location)];
	if (string_to_rank(destination) == 6) {
		taken_pieces.push_back(current_board[index((string_to_rank(destination) - 1), string_to_file(destination))]);
		current_board[index((string_to_rank(destination) - 1), string_to_file(destination))] = std::make_shared<empty>();
	}
	else {
		taken_pieces.push_back(current_board[index(string_to_rank(destination) + 1, string_to_file(destination))]);
		current_board[index((string_to_rank(destination) + 1), string_to_file(destination))] = std::make_shared<empty>();
...
void board::cool_move() {
	...
	else if (move_type == 'E') { std::cout << "Advanced move detected: En Passant!" << std::endl; }
}
...
void board::load_game() {
	...
	std::cout << "Please enter the name of the game you want to load (no extension needed) \nName: ";
